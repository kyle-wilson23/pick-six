...
**Approach:** Team tap only updates a local draft selection. A viewport-fixed floating button (lower-right, mobile + desktop) confirms and runs the existing submit flow (same POST + success messaging). The button stays disabled until the draft differs from the saved pick (or until the first selection when none is saved), and becomes a primary action when dirty.
...
- Disabled (non-primary) when there is no dirty draft: no saved pick and no selection, or draft equals saved (`teamId` + `antiJailedBonus`).
- Enabled primary when draft ≠ saved (including first pick after selecting a team).
...
- Hiding the button entirely when a pick is already saved (vs disabled-until-dirty).
...
|----------|--------------|---------------------------|----------------|
| First pick | No saved pick; user selects team | Draft highlights; FAB enabled primary with team name; no POST yet | N/A |
| Confirm first pick | Dirty draft; user taps FAB | POST; success toast; banner shows saved pick; FAB returns disabled | On API/network error: keep draft, show existing error toast, FAB stays enabled |
| Change pick | Saved pick A; user selects team B | Draft B highlighted; FAB enabled; banner still shows A until confirm | N/A |
| Re-select saved | Draft equals saved (incl. same bonus flag) | FAB disabled / non-primary | N/A |
| Anti-jailed | User chooses 2 PTS chip vs side tap | Draft stores `antiJailedBonus`; confirm POSTs that flag; dirty if bonus differs from saved | Blocked jailed direct pick still shows error toast; no FAB enable |
...
**Execution:**
- [x] `src/components/picks/WeekMatchupList.tsx` -- Split draft selection from saved pick; team select only updates draft + clears blocking errors; POST only from floating confirm; render fixed lower-right Button (primary when dirty, disabled otherwise / while submitting); hide when preview or locked; offset above mobile bottom nav; wire label `Submit Pick: {Team Name}` -- core behavior change.
...
- Given a saved pick, when the page loads, then the floating button is disabled; when the user selects a different team (or changes anti-jailed bonus), then the button becomes enabled primary.
...
**Manual checks:**
- Picks page, unlocked week: tap team → FAB enables → scroll → FAB stays lower-right → submit → toast + banner.
- Already-saved pick: FAB disabled until different team/bonus.
...
- Dirty helper gates FAB enablement (team or bonus change).
...
- Fixed lower-right FAB; primary when dirty; clears bottom nav.
