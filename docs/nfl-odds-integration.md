...
| **Results (finalize scores)** | **The Odds API** `/scores?daysFrom=3` — Tuesday week-close, Saturday `GET/POST /api/cron/sync-nfl-results` (sync only; persist TNF), and Wednesday same route (Wed UTC / Wed ET window; Wednesday also `finalizeNflWeek` for the closed week) plus admin override `POST /api/admin/nfl/sync-results` and settings **Save result as FINAL**. Lookback is **max 3 days**. Missed Saturday → admin **Save result as FINAL** (Tue/Wed Odds cannot recover TNF). |
...
| **Results sync** | **`src/lib/nfl/sync-nfl-results-from-odds.ts`**, cron **`/api/cron/sync-nfl-results`** (Sat sync only; Wed catch-up finalize), admin override **`POST /api/admin/nfl/sync-results`** (Odds `/scores?daysFrom=3`) |
