...
_From `/bmad-code-review` (2026-07-19), reviewing commit `426e2a2` against this story — Blind Hunter + Edge Case Hunter + Acceptance Auditor._

- [x] [Review][Patch] (resolved decision — fixed) Email circuit breaker doesn't span "remaining members/leagues for that invocation" (AC5) — `createEmailCircuitBreaker()` is instantiated fresh inside each `sendTuesdayDigest`/`sendReminder` call, which runs once per league inside the cron route's loop, so a full Resend outage still burns a fresh 3-failure budget on every subsequent league instead of aborting the whole invocation. **Fixed:** breaker is now created once per cron invocation and threaded into every per-league call; the route skips the league entirely (no data fetch, no send attempt) once the circuit is open, and `mapWithConcurrency`'s `shouldAbort` wiring means members not yet reached are also counted as failed. [`src/lib/email/send-tuesday-digest.ts`, `src/lib/email/send-reminder.ts`, `src/lib/email/circuit-breaker.ts`, `src/app/api/cron/tuesday-email/route.ts`, `src/app/api/cron/wednesday-reminder/route.ts`, `src/app/api/cron/thursday-reminder/route.ts`]
- [x] [Review][Patch] (resolved decision — accepted as known exception) AC1 not fully measured — `docs/performance-budgets.md` had real Lighthouse LCP/TTI numbers only for `/login`; picks and standings were marked "re-measure when signed in" rather than actually measured. **Resolution:** an authenticated Lighthouse run against picks/standings would require handing a live session cookie to the CLI — Kyle accepted this as a documented Known Exception rather than have the automated pass do further credential handling. [`docs/performance-budgets.md`]
- [x] [Review][Patch] (resolved decision — accepted as known exception) AC2 not fully measured — the NFR5 section only asserted a "local expectation" with no actual captured sample. **Done for login:** captured two real `authorize()` samples against the live app + Neon DB — 2096ms cold connection, 727ms warm (the cold sample exceeds the 1s target — documented as a new Known Exception, root-caused to Neon connection-pool cold start, not this story's code). **Accepted as exception for pick submit:** every seed league in this dev environment is pre-season (`SEASON_NOT_READY`), so no real save-transaction sample was available without first activating a season; Kyle accepted this as a documented Known Exception rather than mutate dev league state for a one-off timing check. [`docs/performance-budgets.md`]
- [x] [Review][Patch] `mapWithConcurrency`'s `shouldAbort` option is unit-tested but unused by its real callers — both send loops duplicate a manual `if (breaker.open)` check in the mapper instead of passing `shouldAbort`, leaving the dedicated early-exit mechanism as dead code. **Fixed:** both callers now pass `{ shouldAbort: () => breaker.open }` and the manual per-member check was removed. [`src/lib/email/send-tuesday-digest.ts`, `src/lib/email/send-reminder.ts`]
- [x] [Review][Patch] Theme 48px touch target isn't actually theme-wide — `MuiButton` `sizeSmall` and `MuiIconButton` have no `minHeight`/`minWidth` override, only `sizeMedium`/`sizeLarge` do, though AC3 and the Completion Notes claim theme-wide coverage. **Fixed:** added `sizeSmall: { minHeight: 48 }` to `MuiButton` and `minWidth`/`minHeight: 48` to `MuiIconButton`. [`src/theme/create-app-theme.ts`]
- [x] [Review][Patch] Pick-submit NFR5 logging only covers the transaction outcome — invalid-JSON, schema-validation, CSRF, unauthenticated, and forbidden-membership early returns never call `logEvent`, so a meaningful share of real request latency/errors is invisible to the new instrumentation. **Fixed:** added a `logPickSubmitRejected` helper and wired it into every early-return branch (including the transaction's catch block). [`src/app/api/leagues/[leagueId]/picks/route.ts`]
- [x] [Review][Patch] Login NFR5 logging skips the malformed-credentials guard — `authorize()`'s first early return (non-string email/password) exits before any `logEvent` call. **Fixed:** added a `logEvent` call on that branch. [`src/lib/auth.ts`]
- [x] [Review][Patch] `EmailCircuitBreaker` type omits `threshold`, forcing callers to smuggle it via `& { threshold: number }` / `{ threshold?: number }` intersections instead of declaring it on the base type. **Fixed:** `threshold` is now a first-class field on `EmailCircuitBreaker`. [`src/lib/email/circuit-breaker.ts`]
- [x] [Review][Patch] `docs/deployment.md` env-var redeploy guidance is self-contradictory — states vars are "available at runtime" in the same sentence that requires a redeploy for `CRON_SECRET` changes, with no explanation of the distinction. **Fixed:** rewrote to explain vars are injected at build/deploy time, not live, so any change (including `CRON_SECRET`) needs a redeploy. [`docs/deployment.md`]
- [x] [Review][Defer] Circuit-breaker member-skips are merged into the same `failed` counter as real Resend errors, with no separate counter in the cron JSON body — ops can't distinguish genuine failures from breaker no-ops after an outage. [`src/app/api/cron/tuesday-email/route.ts`] — deferred, observability nice-to-have not required by AC5's letter
- [x] [Review][Defer] Circuit-open log events record `remainingAborted: true` but not the count of members actually skipped. [`src/lib/email/send-tuesday-digest.ts:104-116`, `src/lib/email/send-reminder.ts:100-115`] — deferred, minor observability gap
- [x] [Review][Defer] In-memory weather cache has no proactive eviction of stale/unused keys — low risk given the tiny key space (~32 team/kickoff combos) and serverless instance recycling. [`src/lib/integrations/weather/client.ts:29-30`] — deferred, low-impact at MVP scale
- [x] [Review][Defer] Weather client caches a transient fetch failure (`null`) with the same full 10-minute TTL as a success, hiding weather for up to 10 minutes after a blip. [`src/lib/integrations/weather/client.ts:95-101`] — deferred, weather is explicitly best-effort/optional
- [x] [Review][Defer] `mapWithConcurrency`'s `concurrency` argument isn't validated against `NaN`/0/negative — not reachable today since the only caller passes the hardcoded `EMAIL_SEND_CONCURRENCY = 4`. [`src/lib/email/map-with-concurrency.ts:15`] — deferred, defensive hardening for future callers
- [x] [Review][Defer] `mapWithConcurrency` uses `Promise.all` rather than `Promise.allSettled` over its worker pool — a future mapper that rejects (both current callers self-catch) would reject the whole pool. [`src/lib/email/map-with-concurrency.ts:33`] — deferred, not reachable with current callers
