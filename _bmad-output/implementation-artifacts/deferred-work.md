...
- **Global fixture rows not cleaned by Story 8.7 per-league cascade** — Story 8.3 creates global `NflGame` / `OddsSnapshotRun` (`source: "test_fixture"`) / `NflGameOddsLine` / `NflWeekJailedTeam` rows for simulated weeks. These carry no `leagueId`, so Story 8.7's per-league cascade delete cannot remove them. Story 8.7 must explicitly decide: leave them (harmless if the real season hasn't reached that week yet), or add cleanup (e.g. delete `test_fixture`-sourced snapshot runs and any fixture-only game/jailed rows with no surviving real data) as part of "delete test league." **Reaffirmed by Story 8.4 (2026-07-20):** `applySimulationWeekResults` also writes `status` / `homeScore` / `awayScore` / `finalizedAt` on those same global `NflGame` rows — no new row types, but cleanup scope now includes mutated result state on fixture games.
- **Accepted MVP risk: fixture + real schedule mix for the same `(year, week)`** — If fixture `NflGame` rows are created during rehearsal and Story 3.9 later syncs a different real matchup set for the same week, sync **adds** real games (different natural key) rather than replacing fixtures — the week can then mix fixture and real games. Documented in 8.3 Dev Notes; not solved in this story. Related to the 8.7 cleanup decision above. **Reaffirmed by Story 8.4:** when a mixed week has fixture games finalized but a real game still `SCHEDULED`, unmodified `finalizeNflWeek` correctly skips scoring for the whole week (`allGamesFinalized: false`). Same accepted risk class; flag to 8.7 if observed.
