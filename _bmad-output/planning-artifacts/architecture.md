...
- **Epics/stories:** See **`_bmad-output/planning-artifacts/epics.md`** for user stories and acceptance criteria (solutioning 2026). PRD defines **61 FRs (FR1–FR61)**; epics add operational and UX-adjacent requirements (mid-season start, pre-season preview, logos, rehearsal leagues, pre-season polish / launch hardening).
...
- **Time and timezones:** Deadline “5 minutes before first kick” and **EST-oriented** copy must be implemented in a **server-authoritative** way with clear storage of instants. A **game week** is anchored at **Tuesday 00:00 `America/New_York`**, which defines the pick-window **open** instant (**FR26a**); the league’s *first* competition week instead opens at `firstKickoff − 7 days`. The **deadline** (**FR26**) is purely `firstKickoff − 5 minutes` with **no weekday anchor** — do not reintroduce a Thursday rule, which locked 2026 Weeks 1, 12, and 18 early. Invariant: `windowOpen < deadline < firstKickoff` for every league week. Never gate window-open on kickoff — the deadline always precedes kickoff (see `sprint-change-proposal-2026-09-02.md`).
...
| **API / mutation safety** | **CSRF** for cookie-based session mutations where applicable; **rate limiting** on auth endpoints (NFR12, NFR15) and **high-risk mutators** (e.g. **`DELETE /api/leagues/[id]`** for FR61) | Implemented in **`src/proxy.ts`** with in-memory sliding windows per `src/lib/rate-limit.ts`; **shared store** (e.g. `@upstash/ratelimit`) still recommended before horizontal scale. |
...
| **Vercel Cron on Hobby (free tier)** | Per [Vercel Cron usage](https://vercel.com/docs/cron-jobs/usage-and-pricing): Hobby allows **100 cron jobs per project** (the old per-team cap of 2 was lifted Jan 2026, so job count is not a constraint) but each job runs **at most once per day** — sub-daily expressions **fail at deploy time**; **hourly** or sub-minute schedules **require Pro**. Scheduling has **±59 min** precision on Hobby, and cron expressions are **UTC only**. To get **more than one check per day** on Hobby, register **multiple distinct daily jobs** at different UTC hours — the interval limit is **per job**, not per project. *(Plan confirmed Hobby 2026-09-02; see `sprint-change-proposal-2026-09-02.md`.)* | **Design:** register **daily UTC ticks** in `vercel.json` (currently two: `0 11 * * *` and `0 20 * * *`) and let a **pure helper** decide whether a given league/week is due, rather than encoding the schedule in the cron expression. Reminders are **deadline-anchored**, not weekday-anchored (see FR37/FR38). Job handlers must be **idempotent** — every tick re-evaluates, and `LeagueWeekEmailConfig.*SentAt` prevents duplicate sends. **If** precise Tuesday 6:00 PM ET is violated by drift, **mitigate** with: (a) run a **daily** cron that checks “is it time in `America/New_York`?” and sends, or (b) **external free cron** (e.g. cron-job.org) `POST` to a **secret-protected** API route, or (c) upgrade Vercel Pro for tighter schedules—only if needed. |
