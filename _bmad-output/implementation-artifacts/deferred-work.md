...
- **Resend idempotency rolling window duration unspecified** — `docs/email-provider-decision.md`. The doc claims sends with the same idempotency key are deduplicated "within a rolling window" but doesn't specify the window duration. A retry that falls outside the window will produce a duplicate delivery. Verify the actual window length in Resend API docs at Story 6.1 implementation time; document it or code a retry deadline guard accordingly.
...
- **HTTP 429 retry should be differentiated from transient errors** — `docs/email-provider-decision.md`. When Resend returns 429 (daily cap exhausted), exponential backoff retries will all fail until the cap resets at midnight UTC. Story 6.1 retry loop should detect 429 specifically and schedule a next-day retry rather than burning all attempts in the current window.
- **`RESEND_API_KEY` absent at SDK construction — no startup guard** — `docs/email-provider-decision.md`. `new Resend(undefined)` may silently accept a missing key and only fail at send time with an opaque auth error. Story 6.1 should add an explicit startup assertion: `if (!process.env.RESEND_API_KEY) throw new Error('RESEND_API_KEY is not configured')`.
...
- **Hyphen delimiter in idempotency key ambiguous with hyphenated IDs** — `docs/email-provider-decision.md`. The key format `{type}-{leagueId}-{weekNumber}-{participantId}` uses hyphens as delimiters. If any ID is a UUID (which contains hyphens), the key becomes structurally ambiguous. Revisit at Story 6.1: use a colon or pipe delimiter, or assert that all ID components are non-hyphenated before using this pattern.
